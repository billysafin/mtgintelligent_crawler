...
class MySQLStorePipeline(object):
    ...
    def process_item(self, item, spider):
        sql = "SELECT COUNT(*) as count FROM mtg_reading_eng WHERE published = %s AND title = %s"
        self.cursor.execute(sql, (item['published'], item['title']))
        result = self.cursor.fetchall()
        if result[0]['count'] == 0 and item["title"] != "" and item["link"] != "":
            try:
                self.cursor.execute("""INSERT INTO mtg_reading_eng (published, title, link, date, source_from)
                    VALUE(%s, %s, %s, %s, %s)""",
                    (item['published'],
                    item['title'],
                    item['link'],
